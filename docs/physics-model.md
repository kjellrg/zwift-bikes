...
- **A full simulation**, used for every time actually shown to a rider. It walks through the route in tiny slices (every tenth of a second on most routes, stretching to three tenths on the longest ones), looking up the *real* gradient and *real* surface at that exact point, so climbs, descents, and surface changes are all accounted for individually rather than averaged away.
