...
| The `race` arm of the draft resolver | `resolveDraft` in the same module: the scale for the simulator, the `{ mode: 'race' }` twin for the estimate, and a plain solo as the draft's `solo` |
...
| Request wiring, "saves vs solo" comparison | [`server/utils/recommendPipeline.ts`](../server/utils/recommendPipeline.ts) resolves the request's draft once and times the comparison under `draft.solo`, returning a `physics.race` block; the geometry each ride is timed on comes from [`server/api/recommend/[slug].get.ts`](../server/api/recommend/[slug].get.ts) and [`segments/[slug].get.ts`](../server/api/recommend/segments/[slug].get.ts) |
