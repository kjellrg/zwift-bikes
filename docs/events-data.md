...
- **`officialDistanceKm` / `officialElevationM`**: exactly as the organiser publishes them, display-only. The physics runs on the route's own geometry. When the two diverge the race page shows both, and the group's `curatorNote` has to explain why — that is what downgrades the validator's error to a warning.
  ...
  **Always check the published distance against ours, and treat a divergence on an `eventOnly` route as our bug until proved otherwise.** For most routes Zwift's own `leadInDistance` is right and the organiser is quoting the lap; for some event-only routes it is a placeholder and the organiser is right. Zwift publishes 85 m for Urumaze, Mech Isle Mayhem *and* a Mech Isle running route — one value copied across three routes — while the real pens are ~2 km out, which made every prediction on those routes ~7% fast and cost a week of chasing an imaginary rolling-resistance problem (`docs/race-drafting.md` §5). Corrections live in [`shared/data/routeEventLeadIns.ts`](../shared/data/routeEventLeadIns.ts), one entry per route with its source; the default is always what zwift-data ships. Twenty event-only cycling routes still carry sub-200 m lead-ins that nobody has checked against a published event distance.
