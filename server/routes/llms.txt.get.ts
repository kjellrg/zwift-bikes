...
export default defineEventHandler(async (event) => {
  ...
  const body = [
    ...
    `- [MCP server](${origin}/api/mcp): streamable HTTP, gated at the edge - needs access credentials. \`set_rider_profile\`, then \`recommend_for_route\` or \`recommend_for_segment\`. The best fit for a conversation.`,
