...
import { getSeasonBySlug, primaryRouteSlug } from '../../../shared/utils/events'
...
import { routeCardSilhouette } from '../../utils/routeCardCatalog'
 ...
 * `EventRaceRoute`, never as the surface/elevation dataset behind it. Each
 * race row's picture comes with it: the primary route's Silhouette, the same
 * one the route's card draws (#262).
 */
  ...
  return {
    ...
    rounds: season.rounds.map(round => ({
      ...
      races: round.races.filter(race => !race.hidden).map((race) => {
        const primarySlug = primaryRouteSlug(race)
        const silhouette = primarySlug ? routeCardSilhouette(primarySlug) : undefined
        return {
          ...race,
          ...
          categories: race.categories.map((group): EventRaceCategoryWithRoute => {
            ...
            // `scripts/events/validate-events.mjs` fails the build on it rather than
