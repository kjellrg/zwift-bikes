...
/**
 * One ZRL race. Everything a route page can't know lives here: the date, the
...
const categoryGroupOptions = race.categories.map((group, index) => ({
  label: `${formatCategoryGroup(group.cats)} - ${group.laps} lap${group.laps === 1 ? '' : 's'}`,
  value: index
}))
...
const raceTitle = computed(() => `${season!.seriesName} ${season!.label} Round ${race!.round} Week ${race!.week}`)
const raceHeading = computed(() => `Round ${race!.round} Week ${race!.week}`)
...
/**
 * WTRL's published distance/elevation and this site's own totals (lead-in plus
 * laps of real route data) don't always agree - across 2025/26 Round 4 the
 * distance matched to within ~3% but one race's published elevation ran ~86 m
 * high. Both are shown when they differ rather than quietly picking one: the
 * official figure is what riders see in the event listing, and this site's is
 * what the physics below actually runs on.
 */
...
const routeNamesByCategory = computed(() => race!.categories
  .map(group => `${formatCategoryGroup(group.cats)} on ${group.routeName ?? 'a route to be confirmed'}`)
  .join(', '))
...
/** The other groups' route names, when the field is split across courses. */
const splitRouteSummary = computed(() => {
  if (!hasSplitRoutes(race!)) return undefined
  return race!.categories
    .map(group => `${formatCategoryGroup(group.cats)} ride ${group.routeName ?? 'a route still to be confirmed'}`)
    .join('; ')
})
...
watch([weightKg, heightCm, wkg, laps, selectedRouteSlug, myBikesOnly, verifiedOnly, categoryFilter, bikeSearchDebounced], () => {
...
/** Only meaningful once the race has been run - resolved client-side, see below. */
const isPast = ref(false)
onMounted(() => {
  isPast.value = race!.date < new Date().toISOString().slice(0, 10)
})
...
const faqAnswer = computed(() => {
  ...
  return `${rules}. Over ${laps.value} lap${laps.value === 1 ? '' : 's'} of ${routeData.value.name} (${formatDistance(distanceKm)}), our physics model makes the ${equipment} the fastest legal combo, finishing in ${formatDuration(topCombo.value.finishTimeSec)} (~${formatSpeedKmh(distanceKm, topCombo.value.finishTimeSec)}).`
})
...
useSeoMeta({
  title: () => `Best Bike for ZRL ${raceHeading.value} - ${routeNamesLabel.value} (${formatLabel.value}) | Zwift Best Bike`,
  description: () => hasSplitRoutes(race!)
  ...
  ogDescription: () => hasSplitRoutes(race!)
  ...
  <UContainer class="py-10 space-y-10">
    <div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p class="text-muted">
            {{ formatRaceDate(race!.date) }}<template v-if="routeData">
              - {{ routeData.worldName }}
            </template>
          </p>
        </div>
      ...
      >
        ...
        <p class="text-sm text-muted">
          <template v-if="splitRouteSummary">
            {{ splitRouteSummary }} - so the course itself changes with this, not just the distance.
          </template>
          <template v-else>
            Categories race the same route over a different number of laps, so distance, climbing and the fastest combo all change with this.
          </template>
        </p>
      </div>
      ...
      >
        <span class="font-medium text-highlighted">PowerUps:</span> {{ race!.powerups }}
      </p>
    </div>
        ...
        <div class="overflow-x-auto rounded-lg border border-default">
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              >
                <td class="px-4 py-2">
                  <!-- Linked only when the segment has a page here. Several
                       real segments don't: the catalog is built from routes
                       that publish positional data, and not all do. -->
                  <ULink
                    v-if="segment.slug"
    ...
    <div>
      ...
      <UAlert
        ...
        :description="`${season!.organizer} runs ${formatCategoryGroup(selectedGroup?.cats ?? [])} on a route Zwift doesn't publish, so there is no distance, elevation or surface data to simulate against - and a ranking computed from a guess would be worse than none. The published figures above are ${season!.organizer}'s own. Pick another category above to see recommendations for the routes we do have.`"
      />
    ...
    >
      <p class="text-sm text-muted">
        Racing a different number of laps, or want this route outside the league?
        <ULink
