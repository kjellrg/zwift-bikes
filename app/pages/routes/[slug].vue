...
const { data: routeData, error: routeError } = await useFetch(
  () => `/api/routes/${slug.value}`,
);
...
const laps = ref(1);
const lapOptions = Array.from({ length: MAX_LAPS }, (_, i) => ({
  label: `${i + 1} lap${i === 0 ? "" : "s"}`,
  value: i + 1,
}));
const routeTotals = computed(() =>
  routeData.value ? computeRouteTotals(routeData.value, laps.value) : undefined,
);
...
const recommendQuery = computed(() => ({
  ...
  owned: Object.keys(owned.value).length
    ? JSON.stringify(owned.value)
    : undefined,
  ownedWheels: Object.keys(ownedWheels.value).length
    ? JSON.stringify(Object.keys(ownedWheels.value))
    : undefined,
...
const { data: recommendData, status } = await useFetch(
...
const categoryOptions: { label: string; value: BikeCategory | "all" }[] = [
  ...
  { label: BIKE_CATEGORY_LABELS.tt, value: "tt" },
...
const fastestTimeSec = computed(() => {
  const times = combos.value
    .map((c) => c.finishTimeSec)
...
<template>
  <UContainer v-if="routeData" class="py-10 space-y-10">
    <div>
      ...
      <div class="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
        <UCard :ui="{ body: 'text-center py-4' }">
          <p class="text-xs text-muted uppercase tracking-wide">Distance</p>
          <p class="text-xl font-bold">
            {{ formatDistance(routeTotals?.distanceKm ?? routeData.distance) }}
          </p>
        </UCard>
        <UCard :ui="{ body: 'text-center py-4' }">
          <p class="text-xs text-muted uppercase tracking-wide">Elevation</p>
          <p class="text-xl font-bold">
            {{
        ...
        <UCard :ui="{ body: 'text-center py-4' }">
          <p class="text-xs text-muted uppercase tracking-wide">Climb ratio</p>
          <p class="text-xl font-bold">
            {{ routeData.terrain.climbRatio.toFixed(1) }} m/km
          </p>
        </UCard>
        <UCard :ui="{ body: 'text-center py-4' }">
          <p class="text-xs text-muted uppercase tracking-wide">Terrain</p>
          <p class="text-xl font-bold">
      ...
      >
        <div v-if="routeData.lap" class="w-40">
          <label class="block text-xs font-medium text-muted mb-1">Laps</label>
          <USelectMenu
            v-model="laps"
            value-key="value"
            :items="lapOptions"
        ...
        >
          <span class="font-medium text-highlighted">Lead-in:</span>
          {{ formatDistance(routeTotals.leadInDistanceKm)
          }}<template v-if="routeTotals.leadInElevationM > 0">
            / {{ formatElevation(routeTotals.leadInElevationM) }}
          </template>
          (ridden once, not repeated per lap)
        </p>
      </div>
    </div>
    ...
    <div>
      ...
      <div
        class="flex flex-wrap items-end gap-4 rounded-lg border border-default p-4 mb-6"
      >
        <div class="min-w-48">
        ...
        <div class="flex items-center gap-2">
          <USwitch v-model="myBikesOnly" />
          <span class="text-sm">Only show my bikes</span>
          <ULink to="/garage" class="text-sm text-primary underline">
            (edit garage)
          </ULink>
        </div>
      </div>
      ...
      <div
        class="flex flex-wrap items-end gap-6 rounded-lg border border-default p-4 mb-6"
      >
        <div class="w-40">
          <label class="block text-xs font-medium text-muted mb-1"
        ...
        <div class="min-w-64 flex-1">
          <label class="block text-xs font-medium text-muted mb-1">
            Power: {{ wkg.toFixed(1) }} W/kg ({{ Math.round(wkg * weightKg) }}
            W)
          </label>
          <USlider
            :model-value="wkg"
            ...
            :step="0.1"
            @update:model-value="
              (value: number | undefined) => setWkg(value ?? wkg)
            "
          />
        </div>
        <ULink to="/profile" class="text-sm text-primary underline self-center">
          (edit profile)
        </ULink>
      </div>
      ...
      <template v-else>
        ...
        <p v-else-if="!topCombo" class="text-muted text-center py-10">
          No bikes match your filters.
        </p>

        <div class="text-center mt-6">
          <UButton color="neutral" variant="subtle" @click="limit += 9">
            Show more matches
          </UButton>
        </div>
      </template>
