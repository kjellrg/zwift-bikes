 ...
 * onto the one URL we want indexed. This matters most for /segments/[slug]:
 * RouteClimbs/RouteSprints link each segment as `?route=<slug>` once per
 * route it appears on, so a segment like Alpe du Zwift is reachable at dozens
 * of near-identical URLs that would otherwise compete with each other (and
 * with the clean URL) in search results.
 *
 ...
 * neither host we actually render on is the public one: prerendered pages
 * (all of /routes/** and /events/**) are built against `localhost`, and
 * SSR'd pages (/segments/**) see SWA's internal `*.azurewebsites.net`
 * origin rather than zwiftbikes.com. Both leaked into BreadcrumbList
