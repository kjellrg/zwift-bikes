...
const RIDER = { weightKg: 75, heightCm: 183, wkg: 4 } as const
...
describe('estimateFinishTimeSec against the bot-test protocol', () => {
  it('predicts a plausible baseline speed on the flat course', () => {
    const timeSec = estimateFinishTimeSec(FLAT_BOT_COURSE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg)
  ...
  it('reproduces the Tron bike\'s published ~114.6 s/h flat advantage end-to-end', () => {
    ...
    const baseTime = estimateFinishTimeSec(FLAT_BOT_COURSE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg)
    const tronTime = estimateFinishTimeSec(FLAT_BOT_COURSE, tron, undefined, RIDER.weightKg, RIDER.heightCm, RIDER.wkg)
  ...
  it('a fixed-wheel frame ignores whatever wheelset it is passed', () => {
    const tron = classifyBikeFrame(frameByName('Zwift Concept Z1'), 0)
    const withWheels = estimateFinishTimeSec(FLAT_BOT_COURSE, tron, baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg)
    const withoutWheels = estimateFinishTimeSec(FLAT_BOT_COURSE, tron, undefined, RIDER.weightKg, RIDER.heightCm, RIDER.wkg)
    expect(withWheels).toBe(withoutWheels)
  })

  it('more sustained power is always faster', () => {
    const at3 = estimateFinishTimeSec(CLIMB_BOT_COURSE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, 3)
    const at4 = estimateFinishTimeSec(CLIMB_BOT_COURSE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, 4)
    expect(at4).toBeLessThan(at3)
  })
})
...
describe('laps and lead-in', () => {
  ...
  it('adds exactly one lap of time per extra lap (lead-in ridden once)', () => {
    const time = (laps: number) => estimateFinishTimeSec(LEAD_IN_ROUTE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg, laps)
  ...
  it('counts the lead-in on top of the laps', () => {
    ...
    const withLeadIn = estimateFinishTimeSec(LEAD_IN_ROUTE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg, 1)
    const lapOnly = estimateFinishTimeSec(noLeadIn, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg, 1)
  ...
  it('forces one lap on point-to-point routes and clamps at MAX_LAPS on lap routes', () => {
    ...
    const time = (route: RouteWithMeta, laps: number) => estimateFinishTimeSec(route, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg, laps)
...
describe('surface time penalty', () => {
  ...
  it('is zero on a fully paved route', () => {
    const penalty = estimateSurfaceTimePenaltySec(FLAT_BOT_COURSE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg)
  ...
  it('costs a road wheel real time on gravel, and a gravel wheel much less', () => {
    ...
    const roadPenalty = estimateSurfaceTimePenaltySec(GRAVEL_ROUTE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg)
    const gravelPenalty = estimateSurfaceTimePenaltySec(GRAVEL_ROUTE, baselineFrame(), gravelWheels!, RIDER.weightKg, RIDER.heightCm, RIDER.wkg)
...
describe('draft modes', () => {
  it('race draft and TTT paceline are both faster than riding solo, and a bigger team drafts better', () => {
    const solo = estimateFinishTimeSec(FLAT_BOT_COURSE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg)
    const race = estimateFinishTimeSec(FLAT_BOT_COURSE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg, 1, { mode: 'race' })
    const ttt4 = estimateFinishTimeSec(FLAT_BOT_COURSE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg, 1, { mode: 'ttt', riders: 4 })
    const ttt8 = estimateFinishTimeSec(FLAT_BOT_COURSE, baselineFrame(), baselineWheels(), RIDER.weightKg, RIDER.heightCm, RIDER.wkg, 1, { mode: 'ttt', riders: 8 })
