...
- **A wheel that's live in the game but not yet in `zwift-data` goes through `shared/data/wheelSupplement.ts`**, with id/name/imageName copied verbatim from Zwift's game dictionary (`https://www.zwift.com/zwift-web-pages/gamedictionary` - the sole file zwift-data's daily update workflow is generated from), never invented and never taken from ZwiftInsider's sheet spelling (the 2026 Shimano wheels are "Shimano C36" in the game but "Shimano DURA-ACE C36" in the sheet, while the *legacy* wheel keeps the "DURA-ACE" name - keying on the sheet spelling would have silently attached the new wheel's data to the old wheel). The merge prefers upstream on any name or id collision, and `validate-speed-data.mjs` fails the build the moment zwift-data ships a supplemented wheel, naming the entry to delete - a supplement entry is always temporary.
