...
for (const [slug, entry] of Object.entries(data)) {
  ...
  const { entry: normalized, classification } = normalizeRouteSurfaceEntry(entry, route.distance, leadInKm)
...
for (const [classification, slugs] of Object.entries(byClassification)) {
  const detail = ['ride-split', 'ambiguous', 'unknown-slug'].includes(classification) ? `: ${slugs.join(', ')}` : ''
